...
public:
  ...
  //! \param[out] elastic_coherent Thermal elastic coherent scattering cross section 
  ...
  void calculate_xs(double E, double sqrtkT, int* i_temp, double* elastic,
    double* elastic_coherent, double* inelastic, uint64_t* seed) const;
  ...
  bool is_otf_ = false; //!< flag for OTF treatment
  std::unique_ptr<ThermalScatteringOTF> otf_data_; //!< OTF thermal scattering class
