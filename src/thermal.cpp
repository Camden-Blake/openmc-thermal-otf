  ...
  hid_t group, const vector<double>& temperature)
{
  ...
  if (attribute_exists(group, "OTF")){
    is_otf_ = true;
    otf_data_ = std::make_unique<ThermalScatteringOTF>(group);
  }
  else{
    // Read temperatures
...
void ThermalScattering::calculate_xs(double E, double sqrtkT, int* i_temp,
  double* elastic, double* elastic_coherent, double* inelastic, uint64_t* seed) const
{
  if (is_otf_)
  {
    // std::cout << "Calculating OTF XS" << std::endl;
    // *i_temp = 0;
    ThermalScatteringOTF::Thermal_Cross_Sections xs = otf_data_->calculate_xs(E, sqrtkT);
    // std::cout << "Done calculating OTF XS | inelastic = " << xs.inelastic << " | elastic coherent = " << xs.elastic_coherent << " | elastic incoherent = " << xs.elastic_incoherent << std::endl;
    *inelastic = xs.inelastic;
    *elastic_coherent = xs.elastic_coherent;
    *elastic = xs.elastic_coherent + xs.elastic_incoherent;
    // std::cout << "Done copying values | inelastic = " << *inelastic << " | elastic coherent = " << *elastic_coherent << " | elastic = " << *elastic << std::endl;
    assert(*elastic >= 0 && *inelastic >= 0);
    assert(std::isfinite(*elastic) && std::isfinite(*inelastic));
  }
  else{
...
void ThermalScattering::sample(
  const NuclideMicroXS& micro_xs, double sqrtkT, double E_in, double& E_out, double& mu, uint64_t* seed)
{
  if (is_otf_){
    otf_data_->sample_collision(micro_xs, E_in, sqrtkT, E_out, mu, seed);
  }
  else{
    data_[micro_xs.index_temp_sab].sample(micro_xs, E_in, &E_out, &mu, seed);
  }
} 
